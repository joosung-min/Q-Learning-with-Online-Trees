...
iter = 10
n_episode = 500
total_rewards = np.zeros((iter, n_episode))
discards = np.zeros((iter,2))
myMods = {}
...
for i in range(iter):
    ...
    ORFparams = {'minSamples': 256, 'minGain': 0.05, 'xrng': None, 'maxDepth': 15, 'numTrees': 50, 'maxTrees': 100} 
    ...
    # lunar MR 3: memory = 10000, minSamples = 256, minGain=0.05, maxDepth=15
    today = datetime.now()
    backup_file_name = "lunar_MR" + "_3_" + today.strftime("%f")
    ...
    total_rewards[i] = total_reward_episode
    discards[i] = [dqn.a_model[0].discard_rate, dqn.a_model[1].discard_rate]
    myMods[i] = dqn
...
backup_file_p = backup_file_name + ".sav"
...
myEnv["tr"] = total_rewards
myEnv["discards"] = discards

with open(backup_file_name+"_env.sav", "wb") as file:
    pickle.dump(myEnv, file)


pickle.dump(myMods, open(backup_file_name+"_mod.sav", "wb"))

mean_tre = np.mean(total_rewards, axis=0)



plt.plot(mean_tre)
plt.title("(ORF) Mean total reward per episode")
plt.xlabel("Episode")
